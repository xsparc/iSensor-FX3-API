...
#Region "FX3 Board Management"
    ...
    ''' <summary>
    ''' When this function is called, all currently connected FX3 boards are reset to an ADI bootloader state.
    ''' </summary>
    Public Sub ResetAllFX3()
    ...
    ''' <summary>
    ''' The function programs the bootloader of a single board, if there is one available
    ''' </summary>
    Private Sub ProgramBootloader(ByVal selectedBoard As CyFX3Device)
        ...
        Dim devicePath As String = selectedBoard.Path
        ...
        'timer.Start()
        'Dim currentDeviceList As USBDeviceList
        'currentDeviceList = New USBDeviceList(CyConst.DEVICES_CYUSB)
        'While (timer.ElapsedMilliseconds < 5000 And Not progOk)
        '    Thread.Sleep(250)
        '    For Each item As CyUSBDevice In currentDeviceList
        ...
        'If Not progOk Then
        '    Throw New Exception("ERROR: Could not find the FX3 board after programming the bootloader firmware")
        'End If
