...
#Region "Interface Variable Initialization"
    ...
    Private WithEvents m_usbList As USBDeviceList
...
#Region "Connection Functions"
    ...
    ''' <summary>
    ''' Detects all FX3 boards and adds them to a public list of connected devices.
    ''' </summary>
    ''' <param name="Timeout">Optional parameter to configure the board detection timeout</param>
    Public Sub EnumerateFX3(Optional ByVal Timeout As UInt32 = 5000)

        m_usbList = DetectAllFX3(Timeout)
    ...
    ''' </summary>
    ''' <param name="deviceHandle">Device handle for the FX3 to be connected.</param>
    Public Sub Connect(deviceHandle As CyFX3Device)
        ...
        If Not (deviceHandle.bHighSpeed Or deviceHandle.bSuperSpeed) Then
            'Clear the active FX3 device handle
            m_ActiveFX3 = Nothing
        ...
        'Refresh FX3 status and features after programming
        RefreshUSBStatus()
    ...
    ''' </summary>
    ''' <param name="resetHandle">Optional flag to enable/disable resetting the m_ActiveFX3 handle</param>
    ''' <param name="timeout">Optional timeout to wait for a disconnect/reconnect to complete</param>
    Public Sub Disconnect(Optional ByVal resetHandle As Boolean = True, Optional ByVal timeout As UInteger = 3000)
        ...
        If m_ActiveFX3.FriendlyName = "Cypress FX3 USB BootLoader Device" Then
            Exit Sub
        End If

        'Try and send vendor command to disconnect
        ConfigureControlEndpoint(&HB1, True)
        resetComplete = XferControlData(buf, 4, 1000)
        ...
        'Loop here until the selected FX3 board finishes rebooting
        While (timeoutStopwatch.ElapsedMilliseconds() < timeout)
            'Check whether we're in bootloader mode. Exit if we are
            If Not m_ActiveFX3.IsBootLoaderRunning Then
                'Quickly refresh USB to check for changes
                RefreshUSBStatus()
                Thread.Sleep(10)
            Else
                'Set a flag indicating that the reset completed successfully
                resetOk = True
                Exit While
            End If
        End While
        ...
        'Check if we connected to the FX3 and throw an exception if we didn't
        ...
        SetDefaultValues(m_sensorType)
        'Clear the active FX3 device handle if requested
        If resetHandle = True Then
            m_ActiveFX3 = Nothing
        End If

    End Sub
    ''' <summary>
    ''' Refreshes the USB status to update FX3 features after firmware programming.
    ''' </summary>
    Public Sub RefreshUSBStatus()
    ...
    ''' <summary>
    ''' Property which returns the active FX3 board. Can also be used to track if enumeration has been performed. 
    ''' </summary>
    ...
    Public Sub ProgramSDRAM(ByVal FileName As String, Reboot As Boolean)
        ...
        'Attempt to program the board
        Try
            flashSuccess = m_ActiveFX3.DownloadFw(FirmwarePath, FX3_FWDWNLOAD_MEDIA_TYPE.RAM)
        Catch ex As Exception
            m_status = "ERROR: Exception occurred - " + ex.Message
            Throw New Exception(m_status)
        End Try
    ...
    Public Function FX3CodeRunning() As Boolean
        ...
        Try
            'Force a refresh of the usb devices
            RefreshUSBStatus()
            'Read usb device name
            firmwareString = m_ActiveFX3.FriendlyName
        Catch ex As Exception
            firmwareString = ""
            Return False
        End Try
    ...
    Public ReadOnly Property FX3BoardAttached As Boolean
        Get
            ...
            For i As Integer = 0 To devList.Count - 1
                If devList(i).VendorID = &H4B4 Then
                    devList.Dispose()
                    Return True
                End If
            Next
...
#Region "Private Member Functions"
    ...
    ''' <summary>
    ''' Detects and enumerates all FX3 devices connected to the system
    ''' </summary>
    ...
    Private Function DetectAllFX3(ByVal Timeout As Integer) As USBDeviceList

        Dim timeoutStopwatch As New Stopwatch
        Dim boardFound As Boolean = False
        Dim lastException As Exception = Nothing
        Dim tempUsbList As USBDeviceList = Nothing
        ...
        While (timeoutStopwatch.ElapsedMilliseconds() < Timeout) And (Not boardFound)
            Try
                ...
                tempUsbList = New USBDeviceList(CyConst.DEVICES_CYUSB)

                'Check that devices are detected
                If tempUsbList.Count = 0 Then
    ...
    ''' <param name="e"></param>
    Private Sub DisconnectHandler(sender As Object, e As EventArgs) Handles m_usbList.DeviceRemoved
        Dim tempVar As USBDeviceList = sender
        ...
        If m_usbList.Count = 0 And m_FX3Connected Then
            m_FX3Connected = False
            Throw New Exception("ERROR: FX3 Disconnected, no devices found")
        End If

        'There are still devices connected. Refresh USB
        RefreshUSBStatus()
