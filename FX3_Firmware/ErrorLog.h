...
/** The max log capacity for the ring buffer (128KB / 32 bytes per entry -> 4096) */
...
/**
  ...
  * struct is 22 bytes, stored in 32 byte increments.
 **/
typedef struct __attribute__((__packed__)) ErrorMsg
{
	...
	/** The file which originated the error. Is a FileIdentifier cast into short to save flash size */
	uint16_t File;
